...
class Object:
    ...
    def move(self):
        if self.alive == True:
            """if self.submerged > 2:
                if self.speed[0] > 0:
                    self.speed[0] = math.ceil(self.speed[0]/2)
                else:
                    self.speed[0] = math.floor(self.speed[0]/2)
                if self.speed[1] > 0:
                    self.speed[1] = math.ceil(self.speed[1]/2)
                else:
                    self.speed[1] = math.floor(self.speed[1]/2)"""
...
class Player(Object):
    ...
    def playerScript(self):
        ...
        if self.actTimer == 0:       
            ...
            if keys[pygame.K_SPACE]:
                if self.slideItr == 0:
                    if self.grounded:
                        ...
                        if self.submerged > 2 and not keys[pygame.K_UP]:
                            self.flap = True
                            self.swimUp()
                        else:
                            self.float()
            ...
            if keys[pygame.K_LEFT] and self.attack == False and self.slideItr == 0:
                self.dir = "left"
                self.climb = False
            ...
            elif keys[pygame.K_RIGHT] and self.attack == False and self.slideItr == 0:
                self.dir = "right"
                self.climb = False
                """if self.blockedRight == False:
                    self.speed[0] = 1"""
    ...
    """def collisionTests(self):
        if self.collisionCheck(self.top) == True:
            ...
            if self.grounded == False:
                ...
                self.location[1]+=self.distanceToNotCollide(self.top,0,-1)
            elif self.blockedRight == False:
                self.location[1]+=self.distanceToNotCollide(self.top,0,1)
            self.getpoints()
            #else:
        ...
        if self.collisionCheck(self.bottom) == True:
            ...
            if self.blockedTop == False:
                ...
                self.location[1]+=self.distanceToNotCollide(self.bottom,0,-1)
            elif self.blockedRight == False:
                self.location[1]+=self.distanceToNotCollide(self.bottom,0,1)
            self.getpoints()
            #else:
        ...
        if self.collisionCheck(self.left) == True:
            self.blockedLeft == True
            if self.blockedRight == False:
                self.location[1]+=self.distanceToNotCollide(self.left,0,1)
            elif self.blockedTop == False:
                self.location[1]+=self.distanceToNotCollide(self.left,1,-1)
            elif self.blockedRight == False:
                self.location[1]+=self.distanceToNotCollide(self.left,1,1)
            self.getpoints()
            #else:
        ...
        if self.collisionCheck(self.right) == True:
            self.blockedRight == True
            if self.blockedLeft == False:
                self.location[1]+=self.distanceToNotCollide(self.right,0,-1)
            elif self.blockedTop == False:
                self.location[1]+=self.distanceToNotCollide(self.right,1,-1)
            elif self.grounded == False:
                self.location[1]+=self.distanceToNotCollide(self.right,1,1)
            self.getpoints()
            #else:
